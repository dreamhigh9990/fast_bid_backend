...
//**********************************BEGIN MAIN MENU********************************
	jQuery('.page-sidebar li > a .arrow').on('click', function (e) {
		var a_this = $(this).parent();
            if (a_this.next().hasClass('sub-menu') == false) {
                return;
	}
     var parent = a_this.parent().parent();
            ...
            var sub = a_this.next();
            if (sub.is(":visible")) {
                jQuery('.arrow', a_this).removeClass("open");
                a_this.parent().removeClass("open");
                sub.slideUp(200, function () {
                    handleSidenarAndContentHeight();
                });
            } else {
                jQuery('.arrow', a_this).addClass("open");
                a_this.parent().addClass("open");
